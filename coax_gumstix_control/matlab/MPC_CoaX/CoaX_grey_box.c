   ...
   )
{
    ...
    double a_SP, b_SP, c_r, s_r, c_p, s_p, c_y, s_y, z_Tupz, z_Tloz, zeta;
    ...
    // Summarized Moments
    Mx = wq*wr*(Iyy-Izz) - T_up*z_Tup[1]*d_up - T_lo*z_Tlo[1]*d_lo + M_flapup[0] + M_flaplo[0];
    My = wp*wr*(Izz-Ixx) + T_up*z_Tup[0]*d_up + T_lo*z_Tlo[0]*d_lo + M_flapup[1] + M_flaplo[1];
    Mz = wp*wq*(Ixx-Iyy) - k_Mup*Omega_up*Omega_up + k_Mlo*Omega_lo*Omega_lo;
    ...
    Omega_updot  = 1.0/Tf_motup*(Omega_up_des - Omega_up);
    Omega_lodot  = 1.0/Tf_motlo*(Omega_lo_des - Omega_lo);
    
    b_z_bardotz       = 1.0/Tf_up*acos(z_barz)*sqrt(z_barx*z_barx + z_bary*z_bary);
    ...
    z_barxdot = b_z_bardot[0] - wq*z_barz + wr*z_bary;
    z_barydot = b_z_bardot[1] - wr*z_barx + wp*z_barz;
    z_barzdot = b_z_bardot[2] - wp*z_bary + wq*z_barx;
