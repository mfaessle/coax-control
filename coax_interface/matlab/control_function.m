...
I_ra = 8.6700e-005;
I_rr = 4.3350e-005;
A = r*(Izz-Iyy) + (Omega_up - Omega_lo)*(I_ra-I_rr);
B = r*(Ixx-Izz) + (Omega_lo - Omega_up)*(I_ra-I_rr);
...
%F_des = -K_p*e_p - K_v*e_v - K_i*e_i(1:3,1) - Rb2w*[Fx_damp Fy_damp 0]' + m*[xddot_T yddot_T zddot_T]';
F_des(3) = 0;
...
%%%%%%%%%%%%%%%%% Attitude Control
% ep = [x y]' - [x_T y_T]';
% ev = [xdot ydot]' - [xdot_T ydot_T]';
% Kp = 15*eye(2);
% Kv = 10*eye(2);
% pv_err = -Kp*ep - Kv*ev;% + 1/20*m*[xddot_T yddot_T]';
% 
% roll_des = 0;%pv_err(2);
% pitch_des = 0.1;%-pv_err(1);
% 
% K_phi = 3*diag([Ixx Iyy]);
% K_rates = 2*diag([Ixx Iyy]);
% e_phi = [roll pitch]' - [roll_des pitch_des]';
% e_rates = [p q]';
% M_att = -K_phi*e_phi - K_rates*e_rates;
% z_Tlo = zeros(3,1);
% z_Tlo(1) = M_att(2)/(d_lo*k_Tlo*Omega_lo^2);
% z_Tlo(2) = -M_att(1)/(d_lo*k_Tlo*Omega_lo^2);
% z_Tlo(3) = sqrt(1-z_Tlo(1)^2-z_Tlo(2)^2);
%%%%%%%%%%%%%%%%%
